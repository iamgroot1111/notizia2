...
export interface IStorage {
  ...
  addClient(name: string, gender?: Client['gender'], age?: number | null): Promise<void>
  updateClient(id: number, name: string, gender?: Client['gender'], age?: number | null): Promise<void>
  deleteClient(id: number): Promise<void>

  // Cases (Anliegen)
  listCases(clientId: number): Promise<Case[]>
  addCase(c: Omit<Case, 'id' | 'status'> & { status?: Case['status'] }): Promise<void>
  ...
  listAllSessionsExpanded(): Promise<Array<{
    session: Session
    case: Case
    client: Client
  }>>
